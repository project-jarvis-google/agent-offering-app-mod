...
class WorkspaceManager:
    def __init__(self, root_data_dir: str | None = None):
        ...
        self.use_gcs = True
        logger.info(
            f"WorkspaceManager initialized in GCS mode. Bucket: {self.bucket_name}"
        )
    ...
    async def create_workspace(
        ...
        logger.info(
            f"Created workspace {workspace_id} for user {user_id} at {base_path}"
        )
    ...
    async def get_workspace(
        ...
        result = await db.execute(
            select(WorkspaceModel).filter(
                WorkspaceModel.id == workspace_id, WorkspaceModel.user_id == user_id
            )
        )
    ...
    async def list_workspaces(
        ...
        result = await db.execute(
            ...
            .filter(WorkspaceModel.user_id == user_id)
    ...
    async def list_artifacts(
        ...
        if self.use_gcs:
            ...
            prefix = (
                workspace.base_path.replace(f"gs://{self.bucket_name}/", "")
                + "/artifacts/"
            )
        ...
        if self.use_gcs:
            ...
            for b in blobs:
                if b.name.endswith("/"):
                    continue
                files.append(
                    {"name": b.name.split("/")[-1], "size": b.size, "type": "file"}
                )
        else:
    ...
    async def delete_workspace(
        ...
        if not workspace:
            logger.warning(
                f"Delete failed: Workspace {workspace_id} not found or access denied for user {user_id}"
            )
            return False
        ...
        if self.use_gcs:
            try:
                ...
                if blobs:
                    ...
                    logger.info(
                        f"Deleted {len(blobs)} blobs from GCS for workspace {workspace_id}"
                    )
            except Exception as e:
