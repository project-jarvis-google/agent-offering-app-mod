...
def upload_directory_to_gcs(
    local_dir: str, gcs_destination_path: str, max_workers: int = 10
) -> int:
    """
    ...
    logger.info(
        f"Starting upload of {local_dir} to gs://{bucket_name}/{prefix} using {max_workers} workers."
    )
    ...
    for root, _, files in os.walk(local_dir):
        for file in files:
            ...
            gcs_blob_name = os.path.join(prefix, relative_path).replace(
                "\\", "/"
            )  # Normalize for windows paths just in case
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_task):
            _path, blb = future_to_task[future]
            try:
                future.result()
                success_count += 1
    ...
    logger.info(
        f"Successfully uploaded {success_count}/{len(upload_tasks)} files to gs://{bucket_name}/{prefix}"
    )
